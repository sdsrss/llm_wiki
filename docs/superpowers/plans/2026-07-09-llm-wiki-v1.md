...
**Tech Stack:** Node.js ≥22.19（ESM, .mjs）、commander、yaml、turndown + jsdom + @mozilla/readability（html→md 去广告）、mammoth（docx）、pdf-parse（pdf）、node:test（测试，零额外框架）。

## Global Constraints

- Node ≥ 22.19（依赖 commander 15 / undici 8 的实际下限），纯 ESM（`.mjs`），CLI 包位于 `cli/`，npm 包名 `llm-wiki`，bin 名 `llm-wiki`。
...
npm pkg set name=llm-wiki version=0.1.0 type=module license=MIT \
  bin.llm-wiki=./bin/llm-wiki.mjs "engines.node=>=22.19.0" \
