...
npm i commander yaml turndown jsdom @mozilla/readability mammoth pdf-parse
```
...
**Interfaces:**
- Produces: `loadLlmConfig(kbRoot) -> {baseURL, apiKey, model} | null`：合并 `~/.llm-wiki/config.json`（可用 env `LLM_WIKI_CONFIG_DIR` 覆盖目录，测试用）与 kb `wiki.config.json` 的 `llm` 键（kb 优先）；env `LLM_WIKI_API_KEY` 覆盖 apiKey
- Produces: `retrievePages(kbRoot, question, k=6) -> [{relPath, score}]`（BM25 over title+description+tags+body）
- Produces: `askKb(kbRoot, question, {k=6, retrieveOnly=false, fetchImpl=fetch}) async -> {pages, answer|null}`：prompt = index.md + 命中页**全文**，system 要求引用 `[[page]]`；OpenAI 兼容 `POST {baseURL}/chat/completions`
...
export function loadLlmConfig(kbRoot) {
  ...
  if (fs.existsSync(globalFile)) cfg = { ...cfg, ...JSON.parse(fs.readFileSync(globalFile, 'utf8')) }
  const p = kbPaths(kbRoot)
  if (fs.existsSync(p.config)) {
    const kbCfg = JSON.parse(fs.readFileSync(p.config, 'utf8'))
    if (kbCfg.llm) cfg = { ...cfg, ...kbCfg.llm }
  }
  if (process.env.LLM_WIKI_API_KEY) cfg.apiKey = process.env.LLM_WIKI_API_KEY
